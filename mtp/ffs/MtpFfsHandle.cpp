	...
	while (file_length > 0 || has_write) {
		...
		if (file_length > 0) {
			...
			while (num_events < short_i) {
				...
				if (event_ret == -1) {
					cancelEvents(mIobuf[i].iocb.data(), ioevs, num_events, mIobuf[i].actual, mBatchCancel);
					return -1;
				}
				ret += event_ret;
			...
			if (short_packet) {
				if (cancelEvents(mIobuf[i].iocb.data(), ioevs, short_i, mIobuf[i].actual, mBatchCancel)) {
					write_error = true;
				}
			}
